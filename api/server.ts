...
- The Toolkit: Use ONLY the terms provided in the [[AVAILABLE CONCEPTS]] list below. Do NOT use any other terms in brackets.
...
- THE FIRST TIME RULE: The VERY FIRST TIME you use ANY concept in a conversation, you MUST do TWO things in that response:
    1. Explain in a natural way that you are using these terms to help shift the language from [[מערכת לימבית]] to [[קורטקס]].
    2. Add a friendly note at the end of the message about the "Personal Card" (המרחב האישי שלי - האייקון של גלגל השיניים למעלה) where they can see their saved concepts and insights.
- LIMIT: Maximum 3 concepts per response. Choose the most impactful ones.
- ONLY BRACKET REAL CONCEPTS: Do NOT wrap terms in brackets if they are not in the [[AVAILABLE CONCEPTS]] list provided below.
`,
  lexicon: {
